  ...
  <div class="outer">
    ...
    <div class="work_detail" v-if="work !== null && work !== undefined">
      ...
      <div id="detail_description" class="px-5 py-3">
        <label for="" class="font-weight-bold">About this website</label>
        <p class="text-justify">{{work.description}}</p>
      </div>
      <div id="detail_link" class="px-5 py-3" v-if="work.link !== null">
        <label for="" class="font-weight-bold">Interested?</label>
...
export default {
  ...
  methods: {
    ...
    find() {
      let w = this.works.website.find(x => {return x.id == this.$route.params.id})
      this.work = w
    }
  },
